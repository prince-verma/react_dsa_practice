...
describe("then", () => {
  ...
  it("unwraps returned promise in then", () => {
    return promise({ value: 5 })
      .then((v) => promise({ value: v * 2 }))
      .then((v) => expect(v).toEqual(10));
  });

  it("unwraps returned native promise in then", () => {
    return promise({ value: 7 })
...
describe("catch", () => {
  ...
  it("rejects if executor throws", () => {
    return new MyPromise(() => {
      throw "executor error";
    }).catch((e) => expect(e).toEqual("executor error"));
  });
});
...
describe("static methods", () => {
  ...
  it("MyPromise.resolve returns same value if passed a native promise", () => {
    const native = Promise.resolve(42);
    return MyPromise.resolve(native).then((v) => expect(v).toEqual(native));
  });
